(function () {
  'use strict';
  // Tu API Key
  var KEY = "AIzaSyCj7Z_eTo7tOXhZMxqf9GRL_Q-df0G8Jg0",
    // La ID de tu motor de búsqueda
    ENGINE_ID = "014766992644607176084:uurx_qqg5nu",
  ...
  function search(query, count, startIndex) {
    ...
    loadScript(url, {
      ...
      callback: function (data) {
        console.log("por aqui pase y por aqui imprimi"); 
  ...
  function parse(data) {
    ...
    for (; i < len; i++) {
      ...
      output += "<li><h3><a title=\"" + item.snippet.replace(/[<>"]/g, function (a) {
        return {
          "\"": "&quot;",
          "<": "&lt;",
          ">": "&gt;"
        }[a]
      }) + "\" href=\"" + item.link + "\">" + item.htmlTitle + "</a></h3>";
